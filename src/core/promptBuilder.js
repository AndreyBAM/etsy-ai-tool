...
6. SELF-CHECK before you output anything: once you have drafted the title, tags, and description, re-read each of them one more time side by side with the seller's original input. For every material, size, color, price, audience/recipient, style, or certification word that appears in your draft, confirm it was explicitly present in the seller's input. If you find one that was NOT — remove it or replace it with the seller's actual (more general) wording before producing your final answer. Do this check silently; do not show your reasoning, only the final corrected JSON.
