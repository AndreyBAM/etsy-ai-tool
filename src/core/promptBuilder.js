 ...
 *
 * v3.1 (сентябрь 2026) — по итогам ещё одного раунда ручных тестов
 * добавлен явный запрет сравнительных заявлений о превосходстве над
 * массовым/фабричным производством (например "unlike mass-produced
 * pieces") — реальная находка в тесте: продавец ничего не говорил про
 * сравнение с фабричными изделиями, модель сама это добавила в
 * описание. По духу это та же категория выдумки, что и "one of a
 * kind" из правила 2b, — добавлено туда же и в self-check (пункт h).
 * ---------------------------------------------------------------------
 */
...
2b. CRITICAL — do not invent any of the following unless the seller's input explicitly states it: product functions or protective properties (e.g. "protects your jewelry", "keeps it safe"), packaging or gift-ready shipping/presentation claims, engraving/personalization/customization, a claim that each piece is unique or "one of a kind" (handmade alone does not imply this), a comparative superiority claim over mass-produced or factory-made items (e.g. "unlike mass-produced pieces", "you won't find this in factory items" — handmade alone does not imply this comparison either), a specific use case the seller didn't mention, or exact dimensions/finishes/safety features. Do NOT add "for her", "for women", "for men", or any gender- or recipient-specific phrasing to the title, tags, or description unless the seller explicitly named a recipient's gender or identity — a mention that the item "can also be sold as a gift" is NOT permission to assume who it's for. You may make the copy more attractive through tone, structure, and framing, but every concrete product claim must trace back to the seller's original input; if a detail is missing, omit it rather than guessing.

2c. CRITICAL — never infer a more specific product type, construction/mounting method, design style, or descriptive subtype than the seller actually stated, even when it is the single most likely real-world guess. This applies everywhere: title, tags, AND description. Concrete examples of this exact mistake: the seller says "wall shelf" and you write "floating shelf" (a specific mounting style the seller never described); the seller says "wood" and you write "walnut" or "pine" (unless they said that word themselves — see rule 2 above); the seller says "simple" or gives no style word at all and you add "rustic", "minimalist", or "modern"; the seller says "plants" and you write "trailing plant" (a specific plant type/growth habit); the seller says "bag" and you write "tote" or "purse"; the seller says "handmade" and you add "artisan", "luxury", or "premium" as if these were separate facts. The test is simple: if a word in your draft names a subtype, style, material variant, or construction detail that is more specific than what the seller's own words say, delete it or replace it with the seller's actual, more general wording — this holds even if the guess is highly plausible or would make the listing sound better. This rule applies with equal force to every tag individually, not only to the title and description — a short list of tags is not exempt just because it's terse. The only exception is when the seller used that specific word themselves; then it is a stated fact, not an inference, and should be kept.
...
6. SELF-CHECK before you output anything: once you have drafted the title, tags, and description, re-read each of them one more time side by side with the seller's original input. Check specifically for these categories of unstated additions, since they are easy to slip in without noticing: (a) material, size, color, price, style, or certification words; (b) recipient gender ("for her"/"for women"/etc.) or any named audience; (c) a claimed function or protective property; (d) packaging, gift-wrapping, or "ready to ship/gift" claims; (e) engraving, personalization, or customization; (f) a uniqueness claim ("one of a kind", "each piece varies"); (g) a more specific product subtype, style, construction, or mounting method than the seller stated (e.g. "floating" instead of "wall", "rustic"/"minimalist" with no style word given, "walnut"/"pine" with only "wood" given, "trailing plant" with only "plants" given) — check every tag individually for this, not only the title and description; (h) a comparative superiority claim over mass-produced/factory-made items. For every instance in any of these categories, confirm it was explicitly present in the seller's input. If you find one that was NOT — remove it or replace it with the seller's actual (more general) wording before producing your final answer. Do this check silently; do not show your reasoning, only the final corrected JSON.
...
Rewrite the FULL list of ${profile.maxTags} tags, each under ${profile.maxTagLength} characters, fixing ONLY the diversity problem: replace tags so that no keyword (or grammatical variant of it) appears in more than 2 tags total. Keep the tags that were already fine. Replace the excess ones with genuinely different buyer search angles, but ONLY angles grounded in facts already implied by the existing tags/product — do not invent a price positioning, an audience, or a style that isn't already evident. Do not invent a more specific subtype, style, or construction detail than what the product's existing tags/title already state — keep replacement tags as general as the source material. Keep every tag relevant to the same product.
